...
ros::Publisher _pub_ground_cloud_2;
ros::Publisher _pub_ground_cloud_3;
ros::Publisher _pub_ground_intensity_cloud;
ros::Publisher _pub_line_cloud;
...
ros::Publisher _pub_points_lanes_cloud_2;
ros::Publisher _pub_points_lanes_cloud_3;
					...
					float in_right_lane_threshold = 1.5)
{
	...
	for(unsigned int i=0; i< in_cloud_ptr->points.size(); i++)
	{
		...
		if (
				(current_point.x*1.6+current_point.y) > (in_left_lane_threshold) || (current_point.x*1.6+current_point.y) < -1.0*in_right_lane_threshold
			)
		{
...
void removeFloor(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr out_nofloor_cloud_ptr,
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_nofloor_cloud_ptr_2,
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_nofloor_cloud_ptr_3, 
	pcl::PointCloud<pcl::PointXYZI>::Ptr out_onlyfloor_cloud_ptr,
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_onlyfloor_cloud_ptr_2,
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_onlyfloor_cloud_ptr_3,
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_intensity_cloud_ptr, 
float in_max_height=0.05, float in_floor_max_angle=0.1) //0.1 0.3
{

	pcl::PointCloud<pcl::PointXYZI>::Ptr in_tmp_cloud (new pcl::PointCloud<pcl::PointXYZI>);
	pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_floor (new pcl::PointCloud<pcl::PointXYZI>);
	pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_f (new pcl::PointCloud<pcl::PointXYZI>);

	for (unsigned int m=0; m<in_cloud_ptr->points.size(); m++)
	{
		
			in_tmp_cloud->points.push_back(in_cloud_ptr->points[m]);
	}
	...
	int i = 0;
	int nr_points = (int) in_cloud_ptr->points.size ();
    while (in_tmp_cloud->points.size () > 0.3 * nr_points && i <3)
    {
    	seg.setInputCloud(in_tmp_cloud);
	    seg.segment(*inliers, *coefficients);
	    if (inliers->indices.size () == 0)
	    {
		    std::cout << "Could not estimate a planar model for the given dataset." << std::endl;
        ...
        pcl::ExtractIndices<pcl::PointXYZI> extract;
        extract.setInputCloud (in_tmp_cloud);
	    extract.setIndices(inliers);

	    //EXTRACT THE LINE FROM THE CLOUD
	    extract.setNegative(false);//true removes the indices, false leaves only the indices
	    if(i==0)
	       extract.filter(*out_onlyfloor_cloud_ptr);
	    else if(i==1)
	    	extract.filter(*out_onlyfloor_cloud_ptr_2);
	    else
	    	extract.filter(*out_onlyfloor_cloud_ptr_3);
	    ...
	    if(i==0)
	    {
	    	...
	    	*in_tmp_cloud = *out_nofloor_cloud_ptr;
	    }
	       
	    else if(i==1)
	    {
	    	extract.filter(*out_nofloor_cloud_ptr_2);
	    	*in_tmp_cloud = *out_nofloor_cloud_ptr_2;
	    }
	    else
	    {
	    	extract.filter(*out_nofloor_cloud_ptr_3);
	    	*in_tmp_cloud = *out_nofloor_cloud_ptr_3;
	    }
        i++;
        std::cout <<"there are "<<i<<"planes" << std::endl;
    }

	//intensity filter
    for (size_t i = 0; i < out_onlyfloor_cloud_ptr->points.size(); i++) 
        {
    		if(out_onlyfloor_cloud_ptr->points[i].intensity >8 && out_onlyfloor_cloud_ptr->points[i].intensity < 30)
    		{
    			//std::cout << "Intensity:" << out_onlyfloor_cloud_ptr->points[i].intensity << std::endl;
    			out_intensity_cloud_ptr->points.push_back(out_onlyfloor_cloud_ptr->points[i]);
    		}
...
void findLine(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,pcl::PointCloud<pcl::PointXYZI>::Ptr out_line_cloud_ptr,float in_max_distance = 1.5)
{
	pcl::PointCloud<pcl::PointXYZI>::Ptr in_tmp_cloud (new pcl::PointCloud<pcl::PointXYZI>);
	...
	seg.setAxis(Eigen::Vector3f(0,0,1));
	//seg.setEpsAngle(0.3); //0.3
	seg.setEpsAngle(15);
	//seg.setRadiusLimits(0, 0.1);
	seg.setProbability(0.95);
	seg.setDistanceThreshold (0.2);//floor distance 0.1

	//REMOVE THE LINE FROM THE CLOUD
	int i = 0;
	int nr_points = (int) in_cloud_ptr->points.size ();
    while (in_tmp_cloud->points.size () > 0.3 * nr_points && i <10)
    {
    	seg.setInputCloud(in_tmp_cloud);
	    seg.segment(*inliers, *coefficients);
	    if (inliers->indices.size () == 0)
	    ...
	    for (unsigned int j=0; j<cloud_line->points.size(); j++)
	    {

	    	out_line_cloud_ptr->points.push_back(cloud_line->points[j]);
...
void clipCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr out_cloud_ptr, float in_min_height=-2.5, float in_max_height=0.5)
{
	out_cloud_ptr->points.clear();
	for (unsigned int i=0; i<in_cloud_ptr->points.size(); i++)
	{
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
	...
	if (!_using_sensor_cloud)
	{
		...
		pcl::PointCloud<pcl::PointXYZI>::Ptr onlyfloor_cloud_ptr_2 (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZI>::Ptr onlyfloor_cloud_ptr_3 (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZI>::Ptr nofloor_cloud_ptr (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZI>::Ptr nofloor_cloud_ptr_2 (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZI>::Ptr nofloor_cloud_ptr_3 (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZI>::Ptr diffnormals_cloud_ptr (new pcl::PointCloud<pcl::PointXYZI>);
		pcl::PointCloud<pcl::PointXYZ>::Ptr tmp_diffnormals_cloud_ptr (new pcl::PointCloud<pcl::PointXYZ>);
		...
		pcl::PointCloud<pcl::PointXYZI>::Ptr line_cloud_ptr (new pcl::PointCloud<pcl::PointXYZI>);
		...
		if(_remove_ground)
		{
			removeFloor(inlanes_cloud_ptr, nofloor_cloud_ptr, nofloor_cloud_ptr_2, nofloor_cloud_ptr_3, onlyfloor_cloud_ptr,onlyfloor_cloud_ptr_2,onlyfloor_cloud_ptr_3,intensity_cloud_ptr);
			publishCloud(&_pub_ground_cloud, onlyfloor_cloud_ptr);
			publishCloud(&_pub_ground_cloud_2, onlyfloor_cloud_ptr_2);
			publishCloud(&_pub_ground_cloud_3, onlyfloor_cloud_ptr_3);
			publishCloud(&_pub_ground_intensity_cloud, intensity_cloud_ptr);
		}
		else
			nofloor_cloud_ptr = inlanes_cloud_ptr;

		publishCloud(&_pub_points_lanes_cloud, nofloor_cloud_ptr);
		publishCloud(&_pub_points_lanes_cloud_2, nofloor_cloud_ptr_2);
		publishCloud(&_pub_points_lanes_cloud_3, nofloor_cloud_ptr_3);


		findLine(intensity_cloud_ptr, line_cloud_ptr);
		publishCloud(&_pub_line_cloud, line_cloud_ptr);
        ...
        tmp_diffnormals_cloud_ptr->points.resize(diffnormals_cloud_ptr->points.size());
        for (size_t i = 0; i < diffnormals_cloud_ptr->points.size(); i++) 
        {
    		tmp_diffnormals_cloud_ptr->points[i].x = diffnormals_cloud_ptr->points[i].x;
    		tmp_diffnormals_cloud_ptr->points[i].y = diffnormals_cloud_ptr->points[i].y;
    		tmp_diffnormals_cloud_ptr->points[i].z = diffnormals_cloud_ptr->points[i].z;
		}

		segmentByDistance(tmp_diffnormals_cloud_ptr, colored_clustered_cloud_ptr, boundingbox_array, centroids, cloud_clusters, polygon_array, pictograms_array);
...
int main (int argc, char** argv)
{
...
#endif
	_pub_clipped_cloud = h.advertise<sensor_msgs::PointCloud2>("/points_clipped",1);
	_pub_cluster_cloud = h.advertise<sensor_msgs::PointCloud2>("/points_cluster",1);
	_pub_ground_cloud = h.advertise<sensor_msgs::PointCloud2>("/points_ground_1",1);
	_pub_ground_cloud_2 = h.advertise<sensor_msgs::PointCloud2>("/points_ground_2",1);
	_pub_ground_cloud_3 = h.advertise<sensor_msgs::PointCloud2>("/points_ground_3",1);
	_pub_ground_intensity_cloud = h.advertise<sensor_msgs::PointCloud2>("/points_ground_intensity",1);
	_pub_line_cloud = h.advertise<sensor_msgs::PointCloud2>("/points_lines",1);
	...
	_pub_points_lanes_cloud = h.advertise<sensor_msgs::PointCloud2>("/no_floor_1",1);
	_pub_points_lanes_cloud_2 = h.advertise<sensor_msgs::PointCloud2>("/no_floor_2",1);
	_pub_points_lanes_cloud_3 = h.advertise<sensor_msgs::PointCloud2>("/no_floor_3",1);
	...
	private_nh.param("clip_min_height", _clip_min_height, -2.5);	ROS_INFO("clip_min_height: %f", _clip_min_height);
	private_nh.param("clip_max_height", _clip_max_height, -1.5);		ROS_INFO("clip_max_height: %f", _clip_max_height);
